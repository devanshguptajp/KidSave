...
import { ArrowLeft, Plus, Minus, Settings, Lock, Bell, Trash2, DollarSign } from 'lucide-react';
...
export default function ChildDetails() {
  ...
  const [allowanceFrequency, setAllowanceFrequency] = useState<'daily' | 'weekly' | 'custom'>('weekly');
  ...
  const handleAddMoney = () => {
    ...
    setSuccess(`Successfully added ${currencySymbol}${parsedAmount.toFixed(2)}`);
    setAmount('');
    setReason('');
  };
  ...
  const handleSubtractMoney = () => {
    ...
    updateChild(child.id, updatedChild);
    setChild(updatedChild);
    setSuccess(`Successfully subtracted ${currencySymbol}${parsedAmount.toFixed(2)}`);
    setAmount('');
    setReason('');
  };
    ...
    <div className="min-h-screen bg-gradient-to-br from-purple-50 via-blue-50 to-purple-50">
      ...
      <header className="bg-white border-b border-gray-200 sticky top-0 z-10">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
          ...
          <h1 className="text-3xl font-bold text-gray-900">{child.name}'s Account</h1>
          <p className="text-gray-600">Created {new Date(child.createdAt).toLocaleDateString()}</p>
        </div>
      </header>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="bg-gradient-to-br from-blue-600 to-cyan-600 rounded-2xl shadow-lg p-8 mb-8 text-white">
          ...
          <h2 className="text-5xl font-bold mb-6">{currencySymbol}{child.balance.toFixed(2)}</h2>
          ...
          <div className="grid sm:grid-cols-3 gap-4">
            ...
            <div className="bg-white/20 rounded-lg p-4">
              ...
              <p className="text-3xl font-bold">{currencySymbol}{child.piggyBank.toFixed(2)}</p>
            </div>
          </div>
        </div>
                  ...
                  {child.categories.length === 0 ? (
                    ...
                    <div className="grid sm:grid-cols-2 gap-4">
                      {child.categories.map((cat: any) => (
                        <div key={cat.id} className="bg-gradient-to-br from-blue-50 to-cyan-50 rounded-lg p-4">
                          <h4 className="font-bold text-gray-900 mb-2">{cat.name}</h4>
                          <p className="text-2xl font-bold text-blue-600 mb-2">
                            {currencySymbol}{cat.balance.toFixed(2)}
                          </p>
                          {cat.autoSplit && (
                            <span className="text-xs bg-blue-200 text-blue-800 px-2 py-1 rounded">
                              Auto-split: {cat.percentage ? `${cat.percentage}%` : `${currencySymbol}${cat.fixedAmount}`}
                            </span>
                          )}
                        </div>
                      ))}
              ...
              <div className="max-w-md">
                <div className="space-y-6">
                  ...
                  <div className="border-t border-gray-200 pt-6">
                    <h3 className="text-xl font-bold text-gray-900 mb-4">Subtract Money</h3>
                    <div className="space-y-4">
                      <div>
                        <label className="block text-sm font-medium text-gray-700 mb-2">Amount</label>
                        <div className="relative">
              ...
              <div className="max-w-md">
                {child.allowanceAmount ? (
                  <div className="bg-blue-50 border border-blue-200 rounded-lg p-6 mb-6">
                    <h3 className="font-bold text-gray-900 mb-4">Current Allowance</h3>
                    <p className="text-gray-700 mb-2">
                      <strong>Amount:</strong> {currencySymbol}{child.allowanceAmount.toFixed(2)}
                    </p>
                    <p className="text-gray-700 mb-2">
                      <strong>Frequency:</strong> {child.allowanceFrequency}
                    </p>
                    <p className="text-sm text-gray-600">
                      Last paid: {child.lastAllowanceDate ? new Date(child.lastAllowanceDate).toLocaleDateString() : 'Never'}
                    </p>
                  </div>
                ) : null}
                ...
                <div className="space-y-4">
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-2">Allowance Amount</label>
                    <div className="relative">
                      <span className="absolute left-4 top-1/2 transform -translate-y-1/2 text-gray-500 font-semibold">
                        {currencySymbol}
                      </span>
                  ...
                  <div>
                    ...
                    <select
                      ...
                      onChange={(e) => setAllowanceFrequency(e.target.value as any)}
                      className="w-full px-4 py-2 border-2 border-gray-200 rounded-lg focus:outline-none focus:border-blue-600"
                    >
            ...
            {activeTab === 'settings' && (
              <div className="space-y-8 max-w-md">
                ...
                <div>
                  ...
                  {child.pinHash ? (
                    <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-4">
                      <p className="text-gray-700 mb-4">🔒 This account is PIN protected</p>
                      <button
                ...
                <div className="border-t border-gray-200 pt-6">
                  ...
                  <button
                    onClick={() => {
                      if (confirm(`Are you sure you want to delete ${child.name}'s account? This cannot be undone.`)) {
